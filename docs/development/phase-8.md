...
| MODEL-046 | Image-generation jobs: provider interface, `generation_jobs` with RLS, CAS claim, Redis mirror, routes (disabled)   | `packages/ai-core/src/image.ts`, `apps/api/src/modules/image/*`, `src/repositories/generation-job.repository.ts`                                                     | CODE COMPLETE (provider: MODEL-064; in chat: MODEL-065) |
