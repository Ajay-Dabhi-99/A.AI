...
Phases 1–9 were built before the Phase 0 gate closed, by explicit decision on 2026-09-13. None of P0–P9 can be marked DONE until real Supabase, Upstash and at least one provider key are configured and the live suites pass. As of 2026-09-15 the local `.env` database and Redis URLs are placeholders (`127.0.0.1:1`), so the API cannot start locally.
