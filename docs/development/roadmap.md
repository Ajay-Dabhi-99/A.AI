...
| Area                | State                                                                                                                                                                                                                                                                                           | Evidence                                                                                                                                                                 |
| ------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Supabase PostgreSQL | Real project in `.env` (`DATABASE_URL`, `DIRECT_URL`); **all 15 migrations applied** (including `conversation_pin`, `chat_image_jobs`, `user_interests`, `personal_instructions` and `conversation_shares`, 2026-09-17)                                                                         | `pnpm exec prisma migrate status` → "Database schema is up to date"                                                                                                      |
...
| Branch flow (§21.5) | `main` is **protected** (2026-09-22): pull request required, both CI checks must pass on an up-to-date branch, force pushes and deletion blocked, no approving review required (solo maintainer), admins may bypass. Work merges through PRs. No `staging`/`production` GitHub environments yet | GitHub API                                                                                                                                                               |
...
MODEL-069 (2026-09-17): personal instructions, on branch `feature/personal-instructions` (started from `feature/message-actions`).

- API: `users."instructionsAbout"`, `"instructionsStyle"` (`VARCHAR(1500)`) and `"instructionsEnabled"` (default true) in migration `20260917200000_personal_instructions` (matched against `prisma migrate diff`); `GET` and `PATCH /api/me/instructions` (`instructionsUpdateSchema`); `InstructionsService` and `withInstructions` (`modules/users/instructions.service.ts`); `ChatService` builds the system prompt per signed-in request (optional `instructions` dependency).
- Web: `features/profile/instructions-card.tsx` on the profile page (two fields with counters, an on/off switch, save state), `hooks/use-instructions.ts`, and "Personal instructions on" in the chat footer.
- Tests: `packages/validation/tests/instructions.test.ts`, `apps/api/tests/unit/instructions.test.ts`, `apps/api/tests/integration/profile.test.ts` (defaults, save, clear, off, too long, missing field, guests), `apps/api/tests/integration/chat.test.ts` (instructions in the system prompt only while on; never for guests), `apps/web/tests/profile-page.test.tsx` (load, edit, save, too long, save error), `apps/web/tests/chat-page.test.tsx` (footer link).
- Real checks: migration applied; with a temporary Supabase user (deleted afterwards) the service saved, read and switched off the instructions, and Groq answered in one Hindi sentence as instructed.

MODEL-070 (2026-09-17): share links, on branch `feature/share-links` (started from `feature/personal-instructions`).

- API: table `conversation_shares` (migration `20260917220000_conversation_shares`, RLS on, one share per chat, cascades with the chat and the user); `ShareRepository` (`repositories/share.repository.ts`); `ShareService` (`modules/chat/share.service.ts`: text-only snapshot of at most 200 messages with model names, token kept on update); routes in `modules/chat/share.routes.ts`.
