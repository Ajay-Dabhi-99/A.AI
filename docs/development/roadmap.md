...
| Providers           | `GEMINI_API_KEY` set; `GROQ_API_KEY`, `OPENROUTER_API_KEY` empty                                                                                                                                                                                       | `/ready` → `configured: ["gemini"]`                                                      |
...
| 4   | Real provider checks per provider, real comparison with partial failure, real fallback, real summary  | P2, P4–P6  | Owner adds `GROQ_API_KEY` and `OPENROUTER_API_KEY`; single deliberate calls (free-tier limits) |
| 5   | Real verification and reset emails to any address                                                     | P1         | Owner verifies a sending domain in Resend and sets `EMAIL_FROM`                                |
| 6   | Admin promote + disable model; real bucket upload + vision answer; real transcription and voice       | P3, P8, P9 | Manual QA now that the database is live; transcription needs `GROQ_API_KEY`                    |
